...
const canvas = new Canvas(document.getElementById("window"));
const gui = new GUI();
(async () => {
    ...
    const water = canvas.addAsset("tile/water", true, 4, 25);
    const stop = canvas.addAsset("tile/stop");
    const walkingRight = canvas.addAsset("tile/foxWalkingRight", true, 6, 5);
    const walkingLeft = canvas.addAsset("tile/foxWalkingLeft", true, 6, 5);
    const idleRight = canvas.addAsset("tile/foxIdleRight", true, 2, 50);
    const idleLeft = canvas.addAsset("tile/foxIdleLeft", true, 2, 50);
    const treadmil = canvas.addAsset("tile/treadmil", true, 4, 3);
    ...
    setInterval(() => {
        ...
        switch (walkingDir) {
            case 0:
                canvas.drawImage(idleRight, posX, 24);
                break;
            case 2:
                canvas.drawImage(idleLeft, posX, 24);
                break;
            case 4:
                canvas.drawImage(walkingRight, posX, 24);
                break;
            case 6:
                canvas.drawImage(walkingLeft, posX, 24);
                break;
        }
        gui.draw();
